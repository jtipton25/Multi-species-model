multispp.occu.simple.mcmc <- function(y, K=3, nzeroes=100, N=N,
                                      ...
                                      prior.shape.alpha.p=2, prior.rate.alpha.p=1,
                                      prior.shape.beta.p=2, prior.rate.beta.p=1,
                                      prior.shape.alpha.psi=2, prior.rate.alpha.psi=1,
                                      prior.shape.beta.psi=2, prior.rate.beta.psi=1,
  ...
  for(n.mcmc in 1:N.MCMC){
    ...
    p <- rbeta(Omega, apply(Y*z, 1, sum, na.rm=T) + alpha.p, apply(z*(K-Y), 1, sum, na.rm=T) + beta.p)
  ...
  if(drawPlots==T){
    ...
    if (exists("N")==T)  abline(h=N, col="red", lty=2)
    ...
    hist(N.save)
    if (exists("N")==T)  abline(v=N, col="red", lty=2)
    
    par(mfrow=c(2,2) )
    hist(p.alpha.save, main="Det. Prob Alpha")
    curve(dgamma(x, prior.shape.alpha.p, prior.rate.alpha.p), add=T)
    hist(p.beta.save, main="Det Prob Beta")
    curve(dgamma(x, prior.shape.beta.p, prior.rate.beta.p), add=T)
    hist(psi.alpha.save, main="Occu Prob Alpha")
    curve(dgamma(x, prior.shape.alpha.psi, prior.rate.alpha.psi), add=T)
    hist(psi.beta.save, main="Occu Prob Beta")
    curve(dgamma(x, prior.shape.beta.psi, prior.rate.beta.psi), add=T)
  }	
...
multispp.occu.fixedp.mcmc <- function(y, K=3, nzeroes=100, N=N,
                                      alpha.lambda=3, beta.lambda=5, 
  ...
  for(n.mcmc in 1:N.MCMC){
    ...
    #####
    #####  Sample p parameters- we don't in this model.
    #####
    ...
    psi.alpha.save[n.mcmc] <- alpha.psi  
    psi.beta.save[n.mcmc] <- beta.psi  
    #    p.alpha.save[n.mcmc] <- apply(Y*z, 1, sum, na.rm=T) + alpha.p 
    #    p.beta.save[n.mcmc] <- apply(z*(K-Y), 1, sum, na.rm=T) + beta.p 
    N.save[n.mcmc] <- sum(w)
  }
  cat("\n")
